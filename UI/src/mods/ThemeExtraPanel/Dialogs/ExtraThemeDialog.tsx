...
import styles from "./ExtraThemeDialog.module.scss";
...
// ExtraTheme - see that rule's comment). Multiple of our dialogs are never open at once in practice
...
const useDialogOpenClass = () => {
    useEffect(() => {
        if (openCount++ === 0) document.body.classList.add("et-dialog-open");
        return () => {
            if (--openCount === 0) document.body.classList.remove("et-dialog-open");
        };
    }, []);
};
...
// --etDialogMaxWidth/--etDialogMaxHeight (see usePanelDialogBounds.ts); the module's own
...
// fixed-size content (e.g. Export's JSON display) could render larger than the whole ExtraTheme
// panel at its minimum size, spilling past the panel's own edges.
export const ExtraThemeDialog = ({
    title,
    wide,
    buttons,
    onClose,
